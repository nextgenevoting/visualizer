...
From a high level perspective and following the SPA architecture, we have structured our application into 3 main components:
...
\begin{itemize}
	\item The front-end / web-application is where all the functionality of our back-end is consumed and where a typical CHVote election event is visualized for the users. It is therefore the most important component of our application. The backend is developed in response to what is needed by the frontend.
...
Internally, we have divided the VoteService into actors and states: One actor for every protocol participant, providing the functionality a participant is responsible for, and a corresponding state-object for every actor, representing the participants current state within a given election event. 

\begin{center}
\includegraphics[scale=0.68]{assets/voteservice.pdf}
\captionof{figure}{VoteService}\label{VoteService}%
\end{center}
...
\begin{center}
\includegraphics[scale=0.70]{assets/stateClasses.png}
...
\begin{center}
\includegraphics[scale=0.70]{assets/actorClasses.png}

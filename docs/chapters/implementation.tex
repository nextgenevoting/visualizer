...
\end{minted}
On the server side, we have defined a socket.io listener. As the first thing, the handler will remove the calling client from every room before joining the room of the requested election. There is only exception: The client cannot leave the room that corresponds to the `sid` of the request, since this is basically the channel over which the `join` request is handled.
\begin{minted}[linenos,tabsize=2,breaklines]{python}
@socketio.on('join')
def on_join(data):

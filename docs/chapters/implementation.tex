...
\captionof{figure}{JSON patch is an RFC standard that describes a procedure to incrementally patch a JSON object. By comparing two JSON objects JSON patch creates a set of operations that are needed to patch one objects to match the other. We used JSON patch to keep the client side datastores in sync with the central database.}\label{JSON Patch example}%
\end{center}
\end{figure}
